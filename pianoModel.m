...
%%%%% Description:
%%%%%
%%%%% A Kirchhoff Thin Plate FDTD Model Connected to 88 Strings
%%%%% Calibrated to a Piano
...
if losstype == 2
st_hmin = sqrt(st_c.^2*k^2+4*st_sigma1*k +...
    sqrt( (st_c.^2*k^2+4*st_sigma1*k).^2 +16*st_K.^2.*k.^2)) ;
end

%% Alberto (4.47)
% sqrt(st_T*k^2/(2*st_rho*st_A) + k*st_sigma1*.5 + (k*.5)*sqrt((st_T*k/(st_rho*st_A) + 4*st_sigma1)^2 + (16*st_E*st_I)/(st_rho*st_A)))
...
if interpJ
  ...
  %%% Spreading Operators
  J = sparse(Nt,strNum); J(lii+ repmat((0:strNum-1)*Nt,4,1)) = pl_Mr*lic/(1+k*pl_sigma0);

  J(st_w0+ss+(0:strNum-1)*Nt) = -st_Mr./(1+k*st_sigma0);
  pl_J = sparse(ss,strNum); pl_J(lii+ repmat((0:strNum-1)*ss,4,1)) = lic;
