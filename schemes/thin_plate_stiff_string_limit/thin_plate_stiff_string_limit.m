...
pl_I = fidimat(pl_N,pl_N,'I');

pl_mB = -(pl_mu^2)*BH + 2*pl_I;
pl_mC = -pl_I;
...
Dnn = fidimat(st_N,'xx',1);
st_I = speye(st_N);


%%%%% fourth order matrix
Dnn(1,:) = 0; % the end that is coupled has this condition
Dnnnn = Dnn^2;
Dnnnn(end,:) = 0; Dnnnn(:,end) = 0; % set w(end) = 0


%%%%% Alter matrices for coupling boundary

Dnn(1,:) = Dn(1,:);
Dnn(end,:) = 0; Dnn(:,end) = 0; % set w(end) = 0
Dnnnn(1,:) = Dnn(2,:);

%%% String sparsity matrix
st_mB = (st_lambda^2)*Dnn - (st_mu^2)*Dnnnn + 2*speye(st_N);
...
st_Mr = (pl_rho*pl_H*pl_h^2)/((pl_rho*pl_H*pl_h^2)+(st_rho*st_A*st_h));
pl_Mr = (st_rho*st_A*st_h)/((pl_rho*pl_H*pl_h^2)+(st_rho*st_A*st_h));
...
cpl_vc = [-2*pl_mC(pl_w0,:), 2*st_mC(1,:)];   % coupling vector for C matrix
