...
[X,Y] = meshgrid([1:pl_N]*pl_h, [1:pl_N]*pl_h);         % Grid of point in value of meters
pl_dist = sqrt((X-(pl_ctr(1)*pl_L)).^2 + (Y-(pl_ctr(2)*pl_L)).^2); % distance of points from excitation
pl_ind = sign(max(-pl_dist+(pl_wid*0.5),0));         % displaced grid points (logical)
pl_rc = .5*pl_ind.*(1+cos(2*pi*pl_dist/pl_wid));        % displacement
pl_rc = pl_rc(:);                                 % 2D plane as vector

st_X = [1:st_N]*st_h;         % Grid of point in value of meters
st_dist = abs(st_X-st_ctr*st_L); % distance of points from excitation
st_ind = sign(max(-st_dist+(st_wid*0.5),0));         % displaced grid points (logical)
st_rc = .5*st_ind.*(1+cos(2*pi*st_dist/st_wid));        % displacement
st_rc = st_rc(:);                                 % 2D plane as vector
...
%% the backwards time step matrix (loss matrix)
st_mC = -st_I;
...
cpl_vc = [2*J', -2*st_I(1,:)];         % coupling vector for C matrix
...
% u1 = (pl_u0+(k*pl_v0))*pl_rc;
u2 = pl_u0*pl_rc;
u1 = u2;
u  = u2;

w2 = st_w0*st_rc;
w1 = (st_w0+(k*st_v0))*st_rc;
w  = w2;
%%% Joined vectors
uw = [u;w];
uw1 = [u1;w1];
