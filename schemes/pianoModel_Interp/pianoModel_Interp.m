...
% F1 = [-pl_J'*(pl_mI + pl_mC),-st_J'*(st_mI-st_mC);
...
%
% %% coupling points
% pl_w01 = sub2ind([pl_Ny, pl_Nx],floor(pl_ctr*pl_Ny),floor(pl_rx*pl_Nx)*ones(1,strNum));
%
% %%% Mass Ratio Coefficients
% st_Mr1 = (pl_rho*pl_H*pl_h^2).*(1+k*pl_sigma0)./((pl_rho*pl_H*(pl_h^2).*(1+k.*pl_sigma0))+(st_rho.*st_A.*st_h.*(1+k.*st_sigma0)));
% pl_Mr1 = (st_rho.*st_A.*st_h).*(1+k*st_sigma0)./((pl_rho*pl_H*(pl_h^2).*(1+k.*pl_sigma0))+(st_rho.*st_A.*st_h.*(1+k.*st_sigma0)));
% Mr1 = (pl_rho*pl_H*pl_h^2).*(1+k*pl_sigma0).*(st_rho.*st_A.*st_h).*(1+k*st_sigma0)./((pl_rho*pl_H*(pl_h^2).*(1+k.*pl_sigma0))+(st_rho.*st_A.*st_h.*(1+k.*st_sigma0)));
%
% % Main Sparisty Matrices
% B1 = blkdiag(pl_mB,st_mB);
% C1 = blkdiag(pl_mC,st_mC);
%
% % %%% Coupling Vector
...
% B1(pl_w0,:) = B1(pl_w0,:) + spdiags(pl_Mr1',0,strNum,strNum)*cpl_v/(1+k*pl_sigma0);
% B1(ss+1+countN(1:end-1),:) = B1(ss+1+countN(1:end-1),: ) - diag(st_Mr1./(1+k*st_sigma0))*cpl_v;
% % %
% % % %%%careful of signs, exploding systems tend to be due to backward time step
% C1(pl_w0,:) = C1(pl_w0,:) + spdiags(pl_Mr1',0,strNum,strNum)*cpl_vc/(1+k*pl_sigma0);
% C1(ss+1+countN(1:end-1),:) = C1(ss+1+countN(1:end-1),: ) - diag(st_Mr1./(1+k*st_sigma0))*cpl_vc;
%
%
% coFp =  pl_Mr * Mr/(1+k*pl_sigma0);
% coFs =  sparse(diag(st_Mr)) * Mr * sparse(diag(1./(1+k*st_sigma0)));
%
% coFp1 =  spdiags(pl_Mr1',0,strNum,strNum)/(1+k*pl_sigma0);
% coFs1 =  sparse(diag(st_Mr1./(1+k*st_sigma0)));
