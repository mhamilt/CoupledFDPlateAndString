...
for t = 1:3 %%another loop for each tier of strings
  for s = 1:Ts{t}
    ...
    (2*st_sigma1{t}(s)*k/st_h{t}(s)^2)*Dn ) * st_mA ;
...
for t = 1:3
  ...
  pl_w0{t} = sub2ind([pl_Ny, pl_Nx],floor(pl_ctr(Tr)*pl_Ny),floor(pl_rx{t}*pl_Nx)*ones(1,Ts{t}));

  pl_ax{t} = (pl_rx{t}*pl_Nx) - floor(pl_rx{t}*pl_Nx);
  pl_ay{t} = (pl_ctr(Tr)*pl_Ny) - floor(pl_ctr(Tr)*pl_Ny);
  % pl_wL = sub2ind([pl_N, pl_N],floor(pl_ctr(2,1)*pl_N),floor(pl_ctr(2,2)*pl_N));

  %%% Mass Ratio Coefficients
  st_Mr{t} = 1./(st_rho.*st_A{t}.*st_h{t}.*(1+k*st_sigma0{t}));
end

pl_Mr = 1/((pl_rho*pl_H*pl_h^2)*(1+k*pl_sigma0));

%%%Concatenate some cells to vectors
pl_w0 = horzcat(pl_w0{:});
st_Mr = horzcat(st_Mr{:});
pl_ax = horzcat(pl_ax{:});
pl_ay = horzcat(pl_ay{:});
...
%%%% Grid Details
st_ss = sum(horzcat(st_N{:}));
Nt = st_ss + ss;    %total grid points
strNum = fStrNum;   % total string number
...
%%% Mass Ratio Matrix
Mr = inv( spdiags(st_Mr', 0, fStrNum,fStrNum) +...
