...
%   (1-cos((2/itype)*pi.*(dur')/int_dur))*st_d0(notes(iStr)) + ...
...
for s = 1:strNum
  ...
  J([pl_w0(s); pl_w0(s)+1; pl_w0(s)+pl_Nx;pl_w0(s)+pl_Nx+1]+(s-1)*ss) = ...
 [(1-pl_ax)*(1-pl_ay(s));(1-pl_ax)*pl_ay(s);pl_ax*(1-pl_ay(s));pl_ax*pl_ay(s)];

end
% J = sparse(ss,strNum);
% J(pl_w0+(0:strNum-1)*ss) = 1;
...
%%% Coupling Vector
b = (st_rho.*st_A.*st_h)'.*[-J'*pl_mB,st_mB(1+countN(1:end-1),:)];        % coupling vector for B matrix
bc = (st_rho.*st_A.*st_h)'.*[J'*(2*pl_mI + (2*pl_sigma1*k/pl_h^2)*LA)/(1+k*pl_sigma0),...
 -2*st_mI(1+countN(1:end-1),:)./(1+k*st_sigma0)'];   % coupling vector for C matrix

%%% Force Matrix
Fc = (st_rho.*st_A.*st_h)./(pl_rho*pl_H*pl_h^2);
Fm = eye(strNum) + Fc'.*J'*J;
iFm = inv(Fm);

%%%% Jacobi Matrices
di = (0:strNum-1)*strNum + (1:strNum);
dd = Fm(di);
%%%OR
dd = diag(Fm);
iD = sparse(strNum,strNum); iD(di) = 1./dd;
R = sparse(Fm - diag(dd));
...
if run
  ...
  for n = 1:Nf
    ...
    %%%% Jacobi Loop
    % current time step
    % while Fm*a - b*uw1 > eps
    %   a = iD*(b*uw1 - R*a1);
    %   a1 = a;
    % end
    %
    % % previous time step
    % while Fm*ac - bc*uw2 > eps
    %   ac = iD*(b*uw2 - R*a1);
    %   ac1 = ac;
    % end
    % a = ones(88,1);     % current time step

    while Fm*a - (b*uw1 + bc*uw2) > eps
      a = iD*(b*uw1 - R*a1);
      a1 = a;
    end
    ...
    uw = B*uw1 + C*uw2 + fvect + [Jp*a;-Js*a];

    % uw = B*uw1 + C*uw2 + fvect + ...
    % [Jp*(Fm*a);-Js*(Fm*a)] + [Jp*(Fm*ac);-Js*(Fm*ac)];
