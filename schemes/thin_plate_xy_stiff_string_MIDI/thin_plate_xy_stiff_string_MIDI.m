...
% create force signal
st_f = sparse(Nf,strNum);                        % input force signal
...
for s = 1:strNum
  ...
  st_mB{s} = ( (st_lambda(s)^2)*Dnn - (st_mu(s)^2)*Dnnnn + 2*speye(st_N(s)) +...
  (2*st_sigma1/st_h(s)^2)*Dxx ) * st_mA ;
  st_mC{s} = (-(1-st_sigma0*k)*speye(st_N(s))-(2*st_sigma1/st_h(s)^2)*Dxx) * st_mA;
...
save_file = sprintf('%s_xy_stiff.wav',file);
