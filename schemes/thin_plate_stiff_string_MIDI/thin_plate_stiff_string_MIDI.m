...
exc_st_int = floor(exc_st(:,1)*SR) + randi(jiggle,length(exc_st(:,1)),1);          % start time index for excitation
exc_end_int = exc_st_int+durint-1;
for hit = 1:length(exc_st(:,1))
...
for s = 1:strNum
  ...
  (2*st_sigma1*k/st_h(s)^2)*Dn ) * st_mA ;
  st_mC{s} = -((1-st_sigma0*k)*st_mI + (2*st_sigma1*k/st_h(s)^2)*Dnn) * st_mA;
  ...
  st_mC{s}(1,:) = -((1-st_sigma0*k)*st_mI(1,:));
